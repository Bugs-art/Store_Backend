...
def save_product():
    ...
    if "category" not in product:
        return abort(400, "Categoty is required")
...
# create an endpoint that returns a product based on a giv _id
...
def coupon_id(id):
    objId = ObjectId(id)
    coupon = db.Coupons.find_one({"_id"})
    if not coupon:
